...
export function SettingsEditor({ project, issues, runtime, onChange, onDetect, onNew, onSave, onOpen, onExport, onImport, onPlay }: {
  ...
  return (
    <div className="editor-stack">
      ...
      <div className="subsection"><div className="subsection-heading"><div><h3>Play setup</h3><p>Detection is read-only until you explicitly choose Play. Runtime deployment creates a reversible backup.</p></div><div className="button-row"><button className="secondary-button" type="button" onClick={onDetect}>Detect STS2</button><button className="primary-button" type="button" onClick={onPlay}>Play</button></div></div>{runtime ? <div className="runtime-panel"><div><span>Game</span><strong>{runtime.game_found ? 'Found' : 'Not detected'}</strong><small>{runtime.game_path ?? 'No local path detected.'}</small></div><div><span>Game build</span><strong>{runtime.game_version ?? 'Unknown'}</strong><small>{runtime.message}</small></div><div><span>Game support</span><strong>{runtime.base_lib_found && runtime.blank_found ? 'Supported runtime found' : 'Play setup required'}</strong><small>Complete local game support setup before using Play.</small></div></div> : <EmptyState title="Runtime not checked" text="Detect the local game only when you are ready to use Play." action={<button className="secondary-button" type="button" onClick={onDetect}>Detect runtime</button>} />}</div>
